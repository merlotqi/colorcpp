...
Second, several findings are confidence gaps rather than mismatches. `delta_e_94` and `delta_e_cmc` both depend on reference-color asymmetry, yet the tests named `IsGenerallyAsymmetric` only assert non-negativity instead of proving that argument order can matter. `delta_e_2000` is in a better state because the audit included a published-sample subset probe, but the repository still lacks checked-in golden vectors for Sharma's published cases and does not pin the delicate hue-wrap or achromatic branches in permanent tests. The SIMD path is similarly better than average, but its evidence is scalar-equivalence coverage rather than external numeric conformance.
...
| F2 | P1 | The module lacks checked-in golden reference tests for every nontrivial metric, including the default `ΔE2000` path. | `tests/algorithms/test_delta_e.cpp` uses property-style checks for `DeltaE94Test.*`, `DeltaE2000Test.*`, `DeltaECMCTest.*`, and `DeltaEDIN99Test.*`; no published sample tables are embedded there. The audit's Task 2 probe matched six Sharma `ΔE2000` samples, but that evidence is not in the permanent suite. | Future formula regressions can slip through because passing tests only proves broad invariants, not numeric conformance. | Promote published sample vectors into the checked-in tests, starting with Sharma `ΔE2000`, then add the best available references for `ΔE94`, CMC, and DIN99. |
| F3 | P1 | The asymmetric-metric contract is documented but not actually enforced by tests for `delta_e_94()` and `delta_e_cmc()`. | `docs/reference/delta_e.rst` notes `ΔE94` asymmetry; `tests/algorithms/test_delta_e.cpp` lines covering `DeltaE94Test.IsGenerallyAsymmetric` and `DeltaECMCTest.IsGenerallyAsymmetric` only assert both call orders are non-negative. | A refactor could accidentally make these APIs symmetric or swap reference/comparison semantics without obvious test failures. | Replace the current asymmetry smoke tests with explicit argument-order-sensitive examples and add at least one numeric reference case for each metric. |
...
Next, tighten the misuse-prone contracts. Replace the current `ΔE94` and CMC asymmetry smoke tests with cases that actually fail if argument order stops mattering, then add at least one numeric sample per metric so parameter semantics are pinned down. Those changes are still "add validation first" work: they improve confidence without deciding in advance that the implementations are wrong.
