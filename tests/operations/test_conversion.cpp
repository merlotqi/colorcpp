...
TEST(ConversionTest, GraphRoutingPrefersLowerWeightedCanonicalPathToOklab) {
  static_assert(registered_conversion_cost_v<display_p3f_t, rgbf_t> == route_cost::shortcut_4_hop);
  static_assert(registered_conversion_cost_v<rgbf_t, cielab_t> == route_cost::shortcut_2_hop);
  static_assert(graph::minimal_conversion_cost<display_p3f_t, oklab_t>() == 3);
  static_assert(best_route_cost<display_p3f_t, oklab_t>() == 3);
  static_assert(std::is_same_v<graph::next_hop_t<display_p3f_t, oklab_t>, linear_display_p3f_t>);
  static_assert(graph::minimal_conversion_cost<rgbf_t, cielab_t>() == 2);
