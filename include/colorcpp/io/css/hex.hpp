...
inline std::optional<core::rgba8_t> parse_hex(std::string_view s) {
  ...
  if (len == 3) {
    if (details::to_hex_digit(s[0]) < 0 || details::to_hex_digit(s[1]) < 0 || details::to_hex_digit(s[2]) < 0)
      return std::nullopt;
    return core::rgba8_t{details::hex_expand(s[0]), details::hex_expand(s[1]), details::hex_expand(s[2]), 255};
  }
  if (len == 4) {
    for (size_t i = 0; i < 4; ++i)
      if (details::to_hex_digit(s[i]) < 0) return std::nullopt;
    return core::rgba8_t{details::hex_expand(s[0]), details::hex_expand(s[1]), details::hex_expand(s[2]),
                         details::hex_expand(s[3])};
  }
  if (len == 6) {
    for (size_t i = 0; i < 6; ++i)
      if (details::to_hex_digit(s[i]) < 0) return std::nullopt;
    auto a0 = get_val(0), a1 = get_val(1), a2 = get_val(2), a3 = get_val(3), a4 = get_val(4), a5 = get_val(5);
    return core::rgba8_t{static_cast<uint8_t>((*a0 << 4) | *a1), static_cast<uint8_t>((*a2 << 4) | *a3),
  ...
  if (len == 8) {
    ...
    return core::rgba8_t{static_cast<uint8_t>((*v0 << 4) | *v1), static_cast<uint8_t>((*v2 << 4) | *v3),
