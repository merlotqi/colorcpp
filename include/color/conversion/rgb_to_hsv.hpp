...
constexpr core::hsva_int_t convert(const core::basic_rgba<T, Scale>& rgb) {
  ...
  float s = (max_v > 1e-7f) ? (delta / max_v) : 0.0f;
  float v = max_v;

  return core::hsva_int_t(maths::round<int>(h), maths::round<int>(s * 100.0f), maths::round<int>(v * 100.0f),
...
inline constexpr core::hsva_int_t rgb_to_hsv_v = details::convert(RGBType{});
